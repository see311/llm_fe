...
const Welcome = ({ onWelcomeLinkClick, selectedTopic }) => {
  const [activeTab, setActiveTab] = useState('business'); // 'business' 或 'query'
  ...
  const handleQuestionClick = (href, text) => {
    const topic = activeTab;
    onWelcomeLinkClick(href, text, false, topic);
  };

  const followUpQuestions = followUpQuestionsMap[selectedTopic] || followUpQuestionsMap['ssdr-basic'];

  return (
    <WelcomeContainer>
      <Tabs>
        <Tab active={activeTab === 'business'} onClick={() => setActiveTab('business')}>SSDR Business</Tab>
        <Tab active={activeTab === 'query'} onClick={() => setActiveTab('query')}>SSDR Query</Tab>
      </Tabs>
      {selectedTopic && selectedTopic.startsWith('ssdr-') ? (
        <QuestionsGrid>
          {followUpQuestions.map((option, index) => (
            <QuestionCard key={index} onClick={() => handleQuestionClick(option.href, option.text)}>
              {option.text}
            </QuestionCard>
          ))}
        </QuestionsGrid>
